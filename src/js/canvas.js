export const magnification = window.innerHeight / 270;
...
const map = {
	...
	"map2": {
		...
		"place": [
			...
			{
				"placeName": "ROCK IN KITAFES. 2021",
				"pos": [370, 30],
				"fontSize": 60,
			},
			{
...
let draw = {
	colorRect: (_context, json, floor) => {
		let savedFillStyle = _context.fillStyle;

		let drawRect = (pos, color) => {
			for (let i = 0; i < pos.length + 1; ++i) {
				_context.fillStyle = color;				
				if (i === 0) {
					let [x,y] = pos[0].map(x => x*magnification);
					_context.beginPath();
					_context.moveTo(x, y);
					...
					let [x,y] = pos[i].map(x => x*magnification);
					ctx.lineTo(x, y);
				}
			}
		};
		...
		json.Rect.forEach((x) => 
			drawRect(x.pos, x.color)
		);
		_context.fillStyle = savedFillStyle;
	},
	drawPlace: (_context, x, y, data) => {
		...
		img.onload = () => {
			...
			_context.drawImage(
				...
				width,
				height,
				x * magnification - dw / 2,
				(y + 2) * magnification - dh,
				dw,
				dh,
			);
		};
	...
	drawMap: (_context, json, floor) => {
		...
		draw.colorRect(_context,json,floor);
		...
		{ //draw placeName for map
			...
			for (let i of json.place) {
				...
				_context.fillText(
					i.placeName,
					i.pos[0] * magnification,
					i.pos[1] * magnification,
				);
			}
		...
		if (debug_mode) { //draw Node for map
			let x, y;
	...
	connectNode: (_context, json, node1, node2) => {
		...
		switch (floor) {
			...
			default:
				if (debug_mode) {
					console.log(
						"Error in function connectNode; floor is not on range 1<5",
					);
				}
				return;
		}
...
let utils = {
	NodeFloor: (node) => {
		if (0 < node && node <= utils.FloorVariation(2)) {
			...
			utils.FloorVariation(2) < node && node <= utils.FloorVariation(3)
		) {
			return 2;
		} else if (
			utils.FloorVariation(3) < node && node <= utils.FloorVariation(4)
		) {
			return 3;
		} else if (
			utils.FloorVariation(4) < node && node <= utils.FloorVariation(5)
		) {
			return 4;
	...
	FloorVariation: (Floor) => {
		switch (Floor) {
			...
			case 2:
				return 28;
			case 3:
				return utils.FloorVariation(2) + 23;
			case 4:
				return utils.FloorVariation(3) + 18;
			case 5:
				return utils.FloorVariation(4) + 17;
			default:
				return -1000;
		}
	},
...
export function getColorByClick(x, y) {
	return "red";
}
